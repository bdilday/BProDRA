...
The standard deviations of the predictors for the 5 outcomes are,

``` {r}
theta_df <- lapply(1:5, function(i) {
  mods[[i]]@theta}
  ) %>% unlist %>% 
  array(dim=c(3, 5)) %>% t %>% as.data.frame()
...
The `0.573` linear weight value for balls-in-play for a hit is the average linear weight of singles, doubles, and triples, weighted by their league-wide frequency. In the next iteration of this model I'm planning to split those up in to singles and extra-base hits.
...
gg <- df1 %>% 
  ...
  ungroup() %>% 
  ggplot(aes(x=ilw_runs, y=n, group=player_name)) + 
...
There are `K-1` outcomes because I fix the parameter value for the last outcome to zero. This technique assures that the probabilities for the five mutually exclusive outcomes sums to one. This is applied in the `transformed parameters` block.
